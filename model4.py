...
class Generator(nn.Module):
    ...
    def forward(self, img):
        ...
        if img.shape[2]==256:
            ...
            img1dn=img
            img1 = nn.functional.interpolate(img, scale_factor=2, mode='bicubic',align_corners=True,recompute_scale_factor=True)
            imgup = nn.functional.interpolate(img, scale_factor=4, mode='bicubic',align_corners=True,recompute_scale_factor=True)
...
class Generator4(nn.Module):
    def __init__(self,
        ...
        self.dim_head = dim_head
        self.dropout = dropout
        self.pos_emb1D = nn.Parameter(torch.randn(4096, dim))
        self.Transformer_Encoder1 = GTransformerEncoder(self.dim, blocks, num_heads, dim_head, dropout)
        self.sln_norm = SLN(self.dim)
