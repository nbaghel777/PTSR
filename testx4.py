...
def testdataset(generator,generator2,datasetloc,_logger):
    ...
    with torch.no_grad():
        for data in tqdm(range(tot_batch)):
            ...
            realB2 = nn.functional.interpolate(realB4, scale_factor=.5, mode='bicubic',align_corners=True)
            ...
            # a=T.functional.crop(gen4, 0,0, 128,128)
            # b=T.functional.crop(gen4, 0,129, 128,128)
            # c=T.functional.crop(gen4, 129,0, 128,128)
            # d=T.functional.crop(gen4, 129,129, 128,128)
...
#             ab = torch.cat((gena, genb), 3)
#             cd = torch.cat((genc, gend), 3)
#             gen4 = torch.cat((ab, cd), 2)
            
            #print(ab.shape)
            
            
            gen4 = generator2(gen4)
            gen2 = generator2(realB2)
...
#             plt.imshow(get_concat_h([grounddiff4,genrateddiff4]),cmap='jet').savefig(str(args.weight)+'/diff/'+datasetname+str(i)+'x4.jpg')
#             plt.imshow(get_concat_h([grounddiff2,genrateddiff2]),cmap='jet').savefig(str(args.weight)+'/diff/'+datasetname+str(i)+'x2.jpg')
...
def test(generator,generator2):
    ...
    generator.eval()
    #testdataset(generator,generator2,'../dataset/DIV2K_valid_HR',_logger )
    testdataset(generator,generator2,'../dataset/Set5',_logger )
    testdataset(generator,generator2,'../dataset/Set14',_logger )
    testdataset(generator,generator2,'../dataset/BSD100',_logger )
    # testdataset(generator,generator2,'../dataset/urban100',_logger )
    # testdataset(generator,generator2,'../dataset/manga',_logger )
    #testdataset(generator,generator2,'../dataset/FFHQtest',_logger )
...
if __name__ == '__main__':
    ...
    netG = nn.DataParallel(modelx4.Generator()).to(device)
    netG.load_state_dict(copy.deepcopy(torch.load(str(args.weight)+"/weights/Generator.pth")))   
    netGB = nn.DataParallel(model4.Generator()).to(device)
    # netGB.load_state_dict(copy.deepcopy(torch.load(str("PTSR31AUG+bicubic+s5")+"/weights/Generator.pth")))   
    netGB.load_state_dict(copy.deepcopy(torch.load(str("PTSR31AUG+bicubic+s5+newdiffloss2")+"/weights/Generator.pth")))   
